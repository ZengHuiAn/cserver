...
function showPropertyChange(prop_name,delta,hero_name,time,space)
    ...
    local newprefabs = SGK.ResourcesManager.Load("prefabs/newCapacityTip")
...
function showDlgMsg(msg, confirm, cancel, txtConfirm, txtCancel, time, confirmInfo)
    DispatchEvent("showDlgMsg", {msg = msg, confirm = confirm, cancel = cancel, txtConfirm = txtConfirm, txtCancel = txtCancel, time = time, confirmInfo = confirmInfo})
end
        ...
        HelpView.Group[i][CS.UGUIClickEventListener].onClick = function ( ... )
            if i == 1 then
                ...
                if teamInfo.group == 0 or playerModule.Get().id == teamInfo.leader.pid then
                    EventManager.getInstance():dispatch("MAP_CHARACTER_MOVE_Player", {playerModule.GetSelfID(), 0, 0, 0, true});
                else
            ...
            elseif i == 5 then
                ...
                -- local AppInterface = require "module.Cmd.AppImpl"
                -- AppInterface.test("ttt")
            elseif i == 6 then
                -- local list = {}
                -- list[1] = {pid = 111,character = "aaa"}
                -- list[2] = {pid = 2333,character = "bbb"}
                -- ModuleMgr.Get().MapPlayerModule:AddOrUpdateDatas(list)
                -- ModuleMgr.Get().MapPlayerModule:test()
                -- ERROR_LOG( ModuleMgr.Get().MapPlayerModule.data[1]:character() )
                -- ERROR_LOG( ModuleMgr.Get().MapPlayerModule.data[2]:character() )
                
                --ERROR_LOG(GetModule.MapPlayerModule():GetData())
                --utils.SGKTools.StoryEndEffect("我","和","你")
                -- LoadStory(100000,function ( ... )
                    -- utils.SGKTools.Iphone_18(function ( ... )
                    -- end,function ( ... )
                    -- end,{text = "神秘来电", soundName = "lingsheng"})
                --  end)
            elseif i == 7 then
...
function StoryOptions(data)
    ...
    StoryView.resetBtn[CS.UGUIPointerEventListener].onPointerDown = function ( ... )
        DispatchEvent("StoryFrameRecall",0.8)
    end
...
rawset(SGK.UIReference, "Instantiate", function(prefab, parent)
    return SGK.UIReference.Setup(UnityEngine.GameObject.Instantiate(prefab.gameObject or prefab, parent));
end)
...
loadfile = function(file, m, env)
    local str = file_cache[file] or SGK.FileUtils.LoadStringFromFile(file);
    file_cache[file] = str;
...
ThreadEvalWithGameObject = function(script, chunkName, behaviour, ...)
    local func = load(script, chunkName, 'bt', setmetatable({
        this = behaviour,
    }, {__index=_G}));
