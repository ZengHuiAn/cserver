...
function TIMELINE_Finished()
    if game.timeline.winner == 1 then
        ...
        --shuchu疲软
        if not script_data.record_2603003 then
            game:API_AddRecord(nil, 2603003)
        end
        --盾没成功
        if not script_data.record_2603002 then
            game:API_AddRecord(nil, 2603002)
        end
        ...
        if dead_count_1 == 0 then
            game:API_AddRecord(nil, 2603010)
        end
    end
end