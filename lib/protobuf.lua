...
function _decode_type_meta:__index(key)
	...
	if not _reader[t] then
		return function(t, k) return rawget(t, k); end
	end

	local bt =debug.traceback()
	local func = assert(_reader[t],key .. '\ntrackback:' .. bt)(msg)
	self[key] = func
	return func
end
...
local function encode_message(CObj, message_type, t)
	...
	for k,v in pairs(t) do
		local func = type[k]
		if k ~= "_CType" and k ~= "_CObj" and func then
			func(CObj, k , v)
		end
	end
end
...
function _encode_type_meta:__index(key)
	local t, msg = c._env_type(P, self._CType, key)
	if _writer[t] then
		local func = assert(_writer[t], self._CType .. "\t" .. key)(msg)
		self[key] = func
		return func
	else
		-- print("encode warning: unknown field", self._CType .. ":" .. key);
		return nil;	
	end
end
